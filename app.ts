...
module App {
	...
	export function process(canvas: HTMLCanvasElement, context: CanvasRenderingContext2D) {
		...
		for (var r = 0; r < segments; r++) {
			for (var c = 0; c < segments; c++) {
				var sx = Math.floor(canvas.width / segments) * c,
					sy = Math.floor(canvas.height / segments) * r,
					w = Math.floor(canvas.width / segments),
					h = Math.floor(canvas.height / segments),
					avg = averageRgb(context.getImageData(sx, sy, w, h));
		...
		if (luminance(bestRgb) < 40000) { // magic number that worked best
	...
	function sortBucket(arr: Array<RGB>, field: string): Array<RGB> {
	...
	function averageRgb(imageData: ImageData): RGB {
		var i = 0,
			r = 0,
			g = 0,
			b = 0;
	...
	function luminance(obj: RGB): number {
	...
	function luminanceHelper(n: number): number {
